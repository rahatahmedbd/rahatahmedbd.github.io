...
import { updateAdminNameAction, updateAdminProfilePhotoAction } from "@/app/actions/cms";
...
export function ProfileForm({ profile, email }: ProfileFormProps) {
  ...
  const handleUpdateDetails = (e: React.FormEvent) => {
    ...
    startDetailsTransition(async () => {
      // 1. Update Name/phone
      const resName = await updateAdminNameAction(fullName, phone || undefined);
      if (!resName.success) {
      ...
      if (avatarUrl !== profile.avatar_url) {
        const resAvatar = await updateAdminProfilePhotoAction(avatarUrl);
        if (!resAvatar.success) {
          setDetailsError(resAvatar.error || "Failed to update profile photo");
          return;
        }
      }

      setDetailsSuccess("Your profile details have been successfully updated!");
      router.refresh();
    });
  };
  ...
  const handleUpdatePassword = (e: React.FormEvent) => {
    ...
    startPasswordTransition(async () => {
      ...
      if (!res.success) {
        setPwError(res.error || "Failed to update password. Check your current password.");
        return;
      }
  ...
  return (
    <div className="space-y-8 max-w-5xl">
      ...
      <Reveal direction="fade">
        <div>
          <h1 className="text-display-sm font-bold tracking-tight">
            <span className="text-gradient">প্রোফাইল সেটিংস (My Profile)</span>
          </h1>
          <p className="text-sm text-fg-soft mt-1">
            আপনার অ্যাডমিন প্রোফাইলের নাম, মোবাইল নম্বর, ড্যাশবোর্ড ছবি এবং পাসওয়ার্ড আপডেট করুন।
          </p>
        </div>
      </Reveal>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* Right column: Edit Details and Change Password */}
          ...
          <Reveal delay={100}>
            <div className="card-surface p-6 sm:p-8 rounded-3xl border border-border/10 bg-surface/30 backdrop-blur shadow-lift space-y-5">
              <div className="flex items-center gap-2 border-b border-border/5 pb-3">
                ...
                <h3 className="font-bold text-fg text-base">ব্যক্তিগত বিবরণী (Personal Info)</h3>
              </div>
