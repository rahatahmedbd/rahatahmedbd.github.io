...
// GateMount removed (caused loading issues from recent RahatVerse update)
...
export const metadata: Metadata = {
  ...
  icons: {
    icon: [
      { url: "/favicon/profile-favicon.png", sizes: "any" },
      { url: "/favicon/profile-favicon.png", sizes: "32x32", type: "image/png" },
      { url: "/favicon/profile-favicon.png", sizes: "48x48", type: "image/png" },
    ],
...
export default async function RootLayout({
  ...
  // Gate removed (caused loading issues from recent update).
  // Always render the standard fast website (no entry gate).
  ...
  return (
    <html lang="bn" suppressHydrationWarning>
      <head>
        {/* Fonts are loaded from Google with preconnect + display=swap.
            next/font is deliberately not used: it fetches at build time, so
            a transient network failure would break the production build. */}
        ...
        <ThemeProvider>
          <LanguageProvider>
            <ToastProvider>
              ...
              {!isDashboardOrAuth && <NavSpacer />}
              {!isDashboardOrAuth && <BackToTop />}
              {!isDashboardOrAuth && <MobileNav />}
              {!isDashboardOrAuth && <InteractionLayer />}
              {/* GateMount removed (caused slow loading / loading spinner issues from recent RahatVerse update) */}
            </ToastProvider>
          </LanguageProvider>
        </ThemeProvider>
      </body>
