...
import { Settings, Map as MapIcon, X } from "lucide-react";
import { EcctrlJoystick } from "ecctrl";

export function GameUI() {
  const { interactionText, showSettings, setShowSettings, playerPosition, settings, updateSettings } = useGameStore();
  ...
  return (
    <div className="absolute inset-0 pointer-events-none z-10 flex flex-col justify-between p-6">
      {/* Top Bar: Minimap & Settings Toggle */}
      <div className="flex justify-between items-start w-full">
        <div className="bg-slate-900/80 backdrop-blur-md border border-slate-700 p-4 rounded-xl flex flex-col gap-2 pointer-events-auto shadow-xl">
          ...
          <div className="relative w-32 h-32 bg-slate-800 rounded-lg overflow-hidden border border-slate-700">
             ...
             />
             {/* Map elements based on player position (Simplified offset representation) */}
             <div 
        ...
        <button 
          ...
          className="p-3 bg-slate-900/80 backdrop-blur-md border border-slate-700 rounded-full text-slate-300 hover:text-white pointer-events-auto hover:bg-slate-800 transition-all shadow-xl"
        >
          <Settings size={24} />
        </button>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center pointer-events-none">
        {interactionText && (
          <div className="bg-black/60 backdrop-blur-sm border border-cyan-500/30 text-white px-6 py-3 rounded-full text-lg font-bold animate-fade-in shadow-[0_0_20px_rgba(6,182,212,0.2)]">
            {interactionText}
          </div>
        )}
      </div>
        ...
        {!isMobile ? (
          <div className="bg-slate-900/80 backdrop-blur-md p-4 rounded-xl border border-slate-700 pointer-events-auto text-sm text-slate-300 shadow-xl">
            ...
            <ul className="space-y-1">
              <li><kbd className="bg-slate-800 px-1 rounded text-cyan-400">W A S D</kbd> Move</li>
              <li><kbd className="bg-slate-800 px-1 rounded text-cyan-400">Shift</kbd> Sprint</li>
              <li><kbd className="bg-slate-800 px-1 rounded text-cyan-400">Space</kbd> Jump</li>
              <li><kbd className="bg-slate-800 px-1 rounded text-cyan-400">E</kbd> Interact</li>
            </ul>
          </div>
        ) : (
